...
# working long after the feature that installed it went away.
# ---------------------------------------------------------------------------
print("nextdns_cloud_ops sweeps away the retired per-uplink DoH switcher:")
_ND_SCRIPT = ("system", "script")
_ND_SCHED = ("system", "scheduler")
_ND_LEGACY = {
    _ND_SCRIPT: [{".id": "*20", "name": "mikromon-nextdns-wan",
                  "source": ":local want ...",
                  "comment": "mikromon:nextdns:wan-switch:abc123"}],
...
_ND_SETTLED_DNS = [{".id": "*1", "use-doh-server": "https://dns.nextdns.io/abc123",
                    "verify-doh-cert": "no",
                    "servers": "9.9.9.9,149.112.112.112",
                    "allow-remote-requests": "true"}]
_ND_SETTLED_NAT = [
...
legacy_api = FakeApi({_NDRES: [{"version": "7.14.3"}], DNS: _ND_SETTLED_DNS,
                      ("ip", "firewall", "nat"): _ND_SETTLED_NAT, **_ND_LEGACY})
swept = F.nextdns_cloud_ops(Pusher(nd_cfg, legacy_api, dry_run=True), "abc123")
removed = [o for o in swept.ops if o.action == "remove"
           and o.path in (_ND_SCRIPT, _ND_SCHED)]
check("a router still carrying the retired switcher has both the script and "
      "its scheduler removed, even though everything else is already correct",
      len(removed) == 2)
check("...reversibly, like every other managed change",
      all(o.inverse is not None and o.inverse.action == "add" for o in removed))

disabled_legacy = F.nextdns_cloud_ops(
    Pusher(nd_cfg, FakeApi({_NDRES: [{"version": "7.14.3"}], DNS: _ND_SETTLED_DNS,
                            **_ND_LEGACY}), dry_run=True), "")
check("disabling sweeps it too — otherwise the scheduler would keep putting "
      "use-doh-server back every minute and silently undo the disable",
      len([o for o in disabled_legacy.ops if o.action == "remove"
           and o.path in (_ND_SCRIPT, _ND_SCHED)]) == 2)

clean_api = FakeApi({_NDRES: [{"version": "7.14.3"}], DNS: _ND_SETTLED_DNS,
                     ("ip", "firewall", "nat"): _ND_SETTLED_NAT})
check("a router that never had the switcher is still a clean no-op — the "
      "sweep must not invent work on the overwhelming majority of routers",
      F.nextdns_cloud_ops(Pusher(nd_cfg, clean_api, dry_run=True),
                          "abc123").empty)

check("a script owned by the Scripts tab is NOT swept — it has a different "
      "comment prefix, and taking someone's own script off their router "
      "would be far worse than leaving a stale one",
      F.nextdns_cloud_ops(
          Pusher(nd_cfg, FakeApi({
              _NDRES: [{"version": "7.14.3"}], DNS: _ND_SETTLED_DNS,
              ("ip", "firewall", "nat"): _ND_SETTLED_NAT,
              _ND_SCRIPT: [{".id": "*30", "name": "mine",
                            "comment": "mikromon:script:mine"}]}),
              dry_run=True), "abc123").empty)

check("the plan reads plainly as nextdns, with no per-uplink wording left",
      "uplink" not in swept.summary and swept.summary.startswith("nextdns"))
