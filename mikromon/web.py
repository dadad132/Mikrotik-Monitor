...
def _wan_uplink_editor(name, cfg, csrf, ifaces=None, online_ifaces=None) -> str:
    ...
    return (f'<div class="box"><h2>WAN uplinks</h2>'
            ...
            f'static route. Either way, changing a Distance pushes it to the '
            f'router with a brief reconnect on that line so it takes effect '
            f'immediately — RouterOS otherwise only applies a changed distance '
            f'on the line\'s next connection.</p>{detect_note}'
